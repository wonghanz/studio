...
const AiSpeakingExaminationInputSchema = z.object({
  ...
  examType: z.enum(['MUET', 'IELTS']).describe('The target exam type (MUET or IELTS).'),
...
const AiSpeakingExaminationOutputSchema = z.object({
  bandScore: z
    .string()
    .describe(
      'The estimated band score for the spoken response (e.g., "Band 6.5" for IELTS, "Band 5" for MUET).'
    ),
  fluencyTips: z
    .string()
    .describe(
  ...
  modelBand5Answer: z
    .string()
    .describe(
      'A model answer at a Band 5 level for the given topic, demonstrating good speaking practices.'
    ),
});
...
const aiSpeakingExaminationPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert MUET/IELTS speaking examiner. Your task is to evaluate the provided audio recording of a student speaking on the topic "{{{topic}}}".

Based on the audio, provide a comprehensive evaluation in JSON format, including:
1.  An estimated band score suitable for a {{{examType}}} exam (e.g., "Band 6.5" for IELTS, "Band 5" for MUET).
...
4.  A concise model answer at a Band 5 level for the topic "{{{topic}}}", demonstrating good speaking practices.
