...
import { ChevronLeft, Zap, CheckCircle2, Circle, Flame, Award, ArrowRight, Loader2 } from 'lucide-react'
...
export default function DailyQuestPage() {
  ...
  useEffect(() => {
    // In a real app, we'd fetch daily progress from Firestore
    // For demo, we'll check local storage for today's completions
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-8 pb-24">
      <header className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-muted-foreground font-medium text-xs uppercase tracking-widest">Build Your Habits</p>
        </div>
      ...
      <Card className="border-none shadow-lg bg-white overflow-hidden">
        <CardContent className="p-8 space-y-4">
          <div className="flex justify-between items-center text-sm font-bold uppercase tracking-widest text-muted-foreground">
            ...
            <span className="text-primary">{completedIds.length} / {DEMO_QUESTS.length} Quests</span>
          </div>
          <Progress value={progress} className="h-3" />
          {progress === 100 && (
            <div className="flex items-center gap-2 text-accent font-bold animate-bounce mt-2">
              <Award className="w-5 h-5" /> 
              Quest completed! You earned 100 Proficiency XP.
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <section className="pt-8 space-y-4">
         ...
         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
           {[
             ...
             { id: 'streak-7', label: '7-Day Master', desc: 'Focus King', icon: Award, unlocked: (streakData?.currentStreak || 0) >= 7 },
             { id: 'writing-10', label: '10 Essays', desc: 'Word Smith', icon: CheckCircle2, unlocked: false },
             { id: 'writing-50', label: '50 Essays', desc: 'Proficient', icon: Zap, unlocked: false },
           ].map((badge) => (
